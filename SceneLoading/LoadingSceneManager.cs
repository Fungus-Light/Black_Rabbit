...
{	
	...
	public class LoadingSceneManager : MonoBehaviour 
	{
		...
		{
            Time.timeScale = 1f;
			LoadingProgressBar.GetComponent<Image>().fillAmount = Maths.Approach(LoadingProgressBar.GetComponent<Image>().fillAmount,_fillTarget,Time.deltaTime*ProgressBarSpeed);
		}
		...
		protected virtual void LoadingComplete() 
		{
			...
			StartCoroutine(Fade.FadeCanvasGroup(LoadingProgressBar,0.1f,0f));
			StartCoroutine(Fade.FadeCanvasGroup(LoadingAnimation,0.1f,0f));
			StartCoroutine(Fade.FadeCanvasGroup(LoadingCompleteAnimation,0.1f,1f));
